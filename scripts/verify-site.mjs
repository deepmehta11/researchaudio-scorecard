...
assert.match(readerShareJs, /installEvidenceCapture\(\{[\s\S]*?trigger: "scroll",[\s\S]*?offer: \{/);
assert.match(readerShareJs, /railSignal: "Free VRAM worksheet"/);
assert.match(readerShareJs, /railAction: "Unlock worksheet"/);
assert.match(readerShareJs, /railSignal: "Free AI decision memo"/);
assert.match(readerShareJs, /railAction: "Unlock memo"/);
assert.match(readerShareJs, /href: leadMagnet\.href/);
...
assert.match(conversionLoopJs, /const captureOffer = \{ \.\.\.DEFAULT_OFFER, \.\.\.offer \}/);
assert.match(conversionLoopJs, /function applyOffer\(rail, offer, isCustomOffer\)/);
assert.match(conversionLoopJs, /offerPriority === "custom" && !isCustomOffer/);
assert.match(conversionLoopJs, /applyOffer\(existingRail, captureOffer, isCustomOffer\)/);
assert.match(conversionLoopJs, /action\.href = offer\.href/);
...
assert.match(partnersHtml, /github\.com\/deepmehta11\/researchaudio-hf-vram-action\?utm_source=partner_kit&amp;utm_medium=tool&amp;utm_campaign=ai_evidence_lab&amp;utm_content=github_action/);
...
assert.match(indexNowWorkflow, /pages: read/);
assert.match(indexNowWorkflow, /repos\/\$\{GITHUB_REPOSITORY\}\/pages\/builds\?per_page=20/);
assert.match(indexNowWorkflow, /select\(\.commit == \$sha\)/);
assert.match(indexNowWorkflow, /status" == "built"/);
